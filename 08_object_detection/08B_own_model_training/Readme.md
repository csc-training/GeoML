...
4. Train the YOLO model for object detection using the data prepared in the earlier steps. We are using a YOLO model from Ultralytics.
    * Open these files, we will go through it in details.
        * Python file: [08B_2_train_model.py](08B_2_train_model.py)
        * HPC batch job file: [08B_2_train_model.sh](08B_2_train_model.sh)
    ...
    * The shell opens in everybody's home directory, to access the files, change working 
      directory:
        * `cd /scratch/project_2019932/students/$USER/GeoML/08_object_detection/08B_own_model_training`
    * See that you are in the right folder:
        * `ls -l`.
        * It should list the files that you see also in Jupyter file panel.
    * Submit a batch job:
        * `sbatch 08B_2_train_model.sh`
...
5. Run inference using the trained model. The predictions are created by submitting batch job: [08B_3_predict.sh](0B8_3_predict.sh). This script runs the Python file: [08B_3_predict.py](08B_3_predict.py) and predicts the vessels from the test image.
...
6. Evaluate model using the [08B_4_evaluate.ipynb](08B_4_evaluate.ipynb) notebook. The script plots the results and computes performance metrics. 
