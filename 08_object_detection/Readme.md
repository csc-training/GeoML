...
The goal is to detect marine vessels from the Baltic Sea, based on Sentinel-2 1C RGB data. The model for object detection is a YOLO-model. 

This folder includes two exercises:

1. [08A_existing_model](08A_existing_model) - use pre-trained model from Huggingface to predict boats based on an Sentinel-2 image. 
2. [08B_own_model_training](08B_own_model_training) - full workflow for own model training: data preparations, model training, running object detection, saving results in GIS-format, estimating the model.
